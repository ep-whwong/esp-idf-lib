 ...
 * @return int32_t 
 */
int32_t SAFEA1_I2C_Init(void)
{
    if (stsafe_a110_init_desc(HwCtx.dev, HwCtx.port, HwCtx.sda_gpio, HwCtx.scl_gpio) == ESP_OK)
 ...
 * @return int32_t 
 */
int32_t SAFEA1_I2C_DeInit(void)
{
    if (stsafe_a110_free_desc(HwCtx.dev) == ESP_OK)
        return STSAFEA_BUS_OK;
 ...
 */
int8_t StSafeA_HW_Init(i2c_dev_t *dev, i2c_port_t port, gpio_num_t sda_gpio, gpio_num_t scl_gpio)
{
    int8_t ret = StSafeA_HW_Probe(&HwCtx, dev, port, sda_gpio, scl_gpio);

    if (/*(HwCtx.IOInit == NULL) ||*/
        (HwCtx.BusInit == NULL) || (HwCtx.BusSend == NULL) || (HwCtx.BusRecv == NULL) || (HwCtx.CrcInit == NULL) || (HwCtx.CrcCompute == NULL) || (HwCtx.TimeDelay == NULL))
    {
        return STSAFEA_BUS_ERR;
    }
...
StSafeA_ResponseCode_t StSafeA_Receive(StSafeA_TLVBuffer_t *pTLV_Buffer, uint8_t CrcSupport)
{
    ...
    if (pTLV_Buffer != NULL)
    {
        ...
        if (status_code != STSAFEA_BUFFER_LENGTH_EXCEEDED)
        {
            if (status_code == (StSafeA_ResponseCode_t)STSAFEA_BUS_OK)
            {
                status_code = (StSafeA_ResponseCode_t)(uint8_t)((uint8_t)pTLV_Buffer->Header & ~(uint8_t)STSAFEA_CMD_HEADER_RMACEN);
            }
            else if (status_code == (StSafeA_ResponseCode_t)STSAFEA_BUS_NACK)
            {
                status_code = STSAFEA_COMMUNICATION_NACK;
        ...
        if ((CrcSupport != 0U) && (status_code == STSAFEA_OK))
        {
            ...
            if (memcmp(&crc, &pTLV_Buffer->LV.Data[pTLV_Buffer->LV.Length - (uint16_t)2], sizeof(crc)) != 0)
            {
...
void StSafeA_Delay(uint32_t msDelay)
{
    ...
    {
        HwCtx.TimeDelay(pdMS_TO_TICKS(msDelay));
    }
}
/**
 * @}
 ...
 */
int8_t StSafeA_HW_Probe(STSAFEA_HW_t *pCtx, i2c_dev_t *dev, i2c_port_t port, gpio_num_t sda_gpio, gpio_num_t scl_gpio)
{
    ...
    HwCtx->TimeDelay = vTaskDelay;
    HwCtx->DevAddr = STSAFEA_DEVICE_ADDRESS;

    HwCtx->dev = dev;
    HwCtx->port = port;
    HwCtx->sda_gpio = sda_gpio;
    HwCtx->scl_gpio = scl_gpio;
...
static int8_t StSafeA_SendBytes(StSafeA_TLVBuffer_t *pInBuffer)
{
    ...
    if (pInBuffer->LV.Data != NULL)
    {
        ...
        {
            status_code = HwCtx.BusSend(((uint16_t)HwCtx.DevAddr) << 1, (uint8_t *)pInBuffer->LV.Data, i2c_length);
...
static int8_t StSafeA_ReceiveBytes(StSafeA_TLVBuffer_t *pOutBuffer)
{
    ...
    if (pOutBuffer->LV.Data != NULL)
    {
        ...
        while ((status_code != STSAFEA_BUS_OK) && (loop <= (STSAFEA_I2C_POLLING_MAX / STSAFEA_I2C_POLLING_STEP)))
        {
            status_code = HwCtx.BusRecv(((uint16_t)HwCtx.DevAddr) << 1, pOutBuffer->LV.Data, response_length + STSAFEA_HEADER_LENGTH + STSAFEA_LENGTH_SIZE);
        ...
        if ((pOutBuffer->LV.Length > response_length) && (status_code == 0))
        {
            ...
            while ((status_code != STSAFEA_BUS_OK) && (loop <= (STSAFEA_I2C_POLLING_MAX / STSAFEA_I2C_POLLING_STEP)))
            {
                status_code = HwCtx.BusRecv(((uint16_t)HwCtx.DevAddr) << 1, pOutBuffer->LV.Data, pOutBuffer->LV.Length + STSAFEA_HEADER_LENGTH + STSAFEA_LENGTH_SIZE);
...
StSafeA_ResponseCode_t StSafeA_Crc16(StSafeA_TLVBuffer_t *pTLV_Buffer)
{
    ...
    if ((pTLV_Buffer != NULL))
    {
        ...
        crc16 = (uint16_t)HwCtx.CrcCompute(&(pTLV_Buffer->Header), 1U, pTLV_Buffer->LV.Data, pTLV_Buffer->LV.Length);
...
int32_t SAFEA1_I2C_Send(uint16_t DevAddr, uint8_t *pData, uint16_t Length)
{
    ...
    ret = i2c_dev_take_mutex(HwCtx.dev);
    if (ret == ESP_OK)
    {
        ret = i2c_dev_write(HwCtx.dev, NULL, 0, pData, Length);
        if (ret == ESP_OK)
            ret = i2c_dev_give_mutex(HwCtx.dev);
    }
...
int32_t SAFEA1_I2C_Recv(uint16_t DevAddr, uint8_t *pData, uint16_t Length)
{
    ...
    if (ret == ESP_OK)
    {
        ret = i2c_dev_read(HwCtx.dev, NULL, 0, pData, Length);
        if (ret == ESP_OK)
            ret = i2c_dev_give_mutex(HwCtx.dev);
    }
